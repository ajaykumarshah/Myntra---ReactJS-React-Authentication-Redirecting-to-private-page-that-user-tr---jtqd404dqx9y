...
const App = () => {
  ...
  const openProductModal = () => {
    ...
    if (modalShirt.name !== undefined) {
      let returnData = []
      returnData.push(<div className="close-box" onClick={() => { setModalShirt({}) }}>&times;</div>)
      let imageData = []
      modalShirt.otherImages.forEach(element => {
        imageData.push(<img className={'modal-image'} width={'200px'} src={element} />)
      });
      returnData.push(<div className="other-image-wrapper">{imageData}</div>)

      let sizeOption = []
      modalShirt.productSize.split(" ").forEach(element => {
        sizeOption.push(<div className={'size-option'}>{element.substr(0, 2)}</div>)
      })
      returnData.push(<div id='modal-meta-data' >

        <h3>{modalShirt.name}</h3>
        <div><i>{modalShirt.description}</i></div>
        <div>{modalShirt.finalPrice ? ('Rs. ' + modalShirt.finalPrice) : ""}</div>
        <div><strike>{modalShirt.strickPrice ? ('Rs. ' + modalShirt.strickPrice) : ""}</strike></div>
        <div style={{ color: 'red' }}>{modalShirt.discount ? (modalShirt.discount + "%") : ""}</div>

        <div className='size-chart-holder'><div className="size-char-heading">Size Chart</div><div className="size-chart-option-holder">
          {sizeOption}</div></div ></div>)
      returnData.push(<button onClick={() => { setShoppingCart([...shoppingCart, modalShirt]) }}>Add to Cart</button>)
      return returnData;
    }
    else return;
  }
  ...
  const updateShirtList = (newfilterList) => {
    ...
    if (newfilterList['searchStr'] !== undefined) {
      preData = preData.filter(e => {
        if (e.name.match(new RegExp(`.*${newfilterList['searchStr']}.*`, 'i')) || e.description.match(new RegExp(`.*${newfilterList['searchStr']}.*`, 'i'))) {
          return true;
        }
        return false;
      })
    ...
    if (newfilterList['sort'] !== undefined) {
      ...
      if (parseInt(value) === 2) {
        ...
        preData = preData.sort((a, b) => (parseInt(a.finalPrice) - parseInt(b.finalPrice)))
        setSelectedSort(value)
  ...
  const openCartModal = () => {
    ...
    return (<div id='cart-modal-content' >
      <div className="close-box" onClick={() => { setShowCart(false) }}>&times;</div>
  ...
  const displayProductTiles = () => {
    ...
    data.forEach(element => {
      i++;
      a.push(<Product data={element} clickHandler={setModalShirt} key={element.name + i} />);
    })
    return a;
  }
  return (
    <div id="main">
      <div id="product-modal-wrapper" style={modalShirt.name ? {} : { display: 'none' }}>
        <div id="product-modal" style={modalShirt.name ? {} : { display: 'none' }}>{openProductModal()}</div>
      </div>
      ...
      <div id={'page-body'}>
        <div className={'filter-holder'}>
          <div className={'gender-list'}>
            Gender:-
            <li><input type='radio' name="gender-grp" value='M' onClick={() => { updateShirtList({ ...filtertList, gender: 'M' }) }} checked={filtertList['gender'] === 'M'}></input>MEN</li>
            <li><input type='radio' name="gender-grp" value='F' onClick={() => { updateShirtList({ ...filtertList, gender: 'F' }) }} checked={filtertList['gender'] === 'F'} />WOMEN</li>
          </div>
        ...
        <div className={'content-holder'}>
          <div className={'sort-holder'}>
            Sort by:-<select value={selectedSort} onChange={(e) => { updateShirtList({ ...filtertList, sort: e.target.value }) }}>
